 ...
 * @since 1.2
 */
public class GSizeRequirements {
     ...
     */
    public GSizeRequirements(int min, int pref, int max, float a) {
        minimum = min;
        preferred = pref;
        maximum = max;
     ...
     */
    public static GSizeRequirements getTiledSizeRequirements(GSizeRequirements[]
                                                            children) {
        GSizeRequirements total = new GSizeRequirements();
        for (int i = 0; i < children.length; i++) {
            GSizeRequirements req = children[i];
     ...
     * @return  the total space requirements.
     */
    public static GSizeRequirements getAlignedSizeRequirements(GSizeRequirements[]
                                                              children) {
        GSizeRequirements totalAscent = new GSizeRequirements();
        GSizeRequirements totalDescent = new GSizeRequirements();
        for (int i = 0; i < children.length; i++) {
            GSizeRequirements req = children[i];
        ...
        return new GSizeRequirements(min, pref, max, alignment);
    }
    ...
    public static void calculateTiledPositions(int allocated,
                                               GSizeRequirements total,
                                               GSizeRequirements[] children,
                                               int[] offsets,
                                               int[] spans) {
    ...
    private static void compressedTile(int allocated, long min, long pref, long max,
        ...
        if( forward ) {
            ...
            for (int i = 0; i < spans.length; i++) {
                ...
                GSizeRequirements req = request[i];
     ...
     */
    public static int[] adjustSizes(int delta, GSizeRequirements[] children) {
      return new int[0];
    }
}
