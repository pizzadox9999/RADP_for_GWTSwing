 ...
 * {@code componentOrientation} property and may be one of two values:
 * <ul>
 ...
 * alignment is determined by the {@code align} property. The possible values
 * are:
 * <ul>
 * <li>{@link #LEFT LEFT}
 * <li>{@link #RIGHT RIGHT}
 ...
 */
public class GFlowLayout implements GLayoutManager {
	...
	/**
	 * {@code align} is the property that determines how each row distributes empty
	 * space. It can be one of the following values:
	 * <ul>
	 * <li>{@code LEFT}
	 * <li>{@code RIGHT}
	 * <li>{@code CENTER}
	...
	/**
	 * {@code newAlign} is the property that determines how each row distributes
	 * empty space for the Java 2 platform, v1.2 and greater. It can be one of the
	 ...
	 * the borders of the {@code Container}.
	 *
	 * @serial
	 * @see #getHgap()
	 * @see #setHgap(int)
	...
	/**
	 * Constructs a new {@code FlowLayout} with a centered alignment and a default
	 ...
	 * argument must be one of {@code FlowLayout.LEFT}, {@code FlowLayout.RIGHT},
	 * {@code FlowLayout.CENTER}, {@code FlowLayout.LEADING}, or
	 * {@code FlowLayout.TRAILING}.
	 * 
	 * @param align the alignment value
	 */
	public GFlowLayout(int align) {
		this(align, 5, 5);
	}
	 ...
	 *              and the borders of the {@code Container}
	 ...
	 * {@code FlowLayout.LEFT}, {@code FlowLayout.RIGHT}, {@code FlowLayout.CENTER},
	 * {@code FlowLayout.LEADING}, or {@code FlowLayout.TRAILING}.
	 * 
	...
	public GDimension preferredLayoutSize(GComponent target) {
		...
		for (int i = 0; i < nmembers; i++) {
			...
			if (m.isVisible()) {
				GDimension d = m.getPreferredSize();
	...
	public GDimension minimumLayoutSize(GComponent target) {
		...
		for (int i = 0; i < nmembers; i++) {
			...
			if (m.isVisible()) {
				...
				dim.width += d.width;
				/*
				 * if (useBaseline) { int baseline = m.getBaseline(d.width, d.height); if
				 * (baseline >= 0) { maxAscent = Math.max(maxAscent, baseline); maxDescent =
				 * Math.max(maxDescent, dim.height - baseline); } }
				 */
			}
		}

		/*
		 * if (useBaseline) { dim.height = Math.max(maxAscent + maxDescent, dim.height);
		 * }
		 */
	 ...
	 */
	private int moveComponents(GComponent target, int x, int y, int width, int height, int rowStart, int rowEnd,
	 ...
	 * order to satisfy the alignment of this {@code FlowLayout} object.
	 *
	...
	public void layoutContainer(GComponent target) {
		//synchronized (target.getTreeLock()) {
			...
			boolean ltr = true; //just assume left to right target.getComponentOrientation().isLeftToRight();
	...
	/**
	 * Returns a string representation of this {@code FlowLayout} object and its
	 * values.
	 * 
