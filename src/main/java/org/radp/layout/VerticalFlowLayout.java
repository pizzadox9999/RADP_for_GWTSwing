 ...
 * @since March 24, 2001
 */
public class VerticalFlowLayout implements GLayoutManager {
	...
	public GDimension preferredLayoutSize(GComponent target) {
		//synchronized (target.getTreeLock()) {
			...
			for (int ii = 0; ii < nmembers; ii++) {
				GComponent m = target.getComponent(ii);
				if (m.isVisible()) {
	...
	public void layoutContainer(GComponent target) {
		//synchronized (target.getTreeLock()) {
			...
			GDimension preferredSize = preferredLayoutSize(target);
			GDimension targetSize = target.getSize();
