 ...
 */
public class RelativeLayout implements GLayoutManager {
	...
	private HashMap<GComponent, Float> constraints = new HashMap<GComponent, Float>();
	 ...
	 */
	public Float getConstraints(GComponent component) {
	 ...
	 * Not supported
	 */
	public void addLayoutComponent(String name, GComponent component) {
	}

	/*
	 * Keep track of any specified constraint for the component.
	 */
	public void addLayoutComponent(GComponent component, Object constraint) {
	 ...
	 */
	private void layoutContainerHorizontally(GComponent parent) {
		...
		for (int i = 0; i < components; i++) {
			GComponent component = parent.getComponent(i);
			...
			if (constraint == null) {
				GDimension d = component.getPreferredSize();
	 ...
	 */
	private int getLocationY(GComponent component, int height) {
		...
		if (alignmentY == COMPONENT)
			alignmentY = GComponentUtil.getAlignmentY(component);
	 ...
	 */
	private void layoutContainerVertically(GComponent parent) {
		int components = parent.getComponentCount();
		int visibleComponents = getVisibleComponents(parent);
	 ...
	 */
	private int getLocationX(GComponent component, int width) {
		// Use the Container alignment policy
		...
		if (alignmentX == COMPONENT)
			alignmentX = GComponentUtil.getAlignmentX(component);
	 ...
	 */
	private int[] allocateRelativeSpace(GComponent parent, int spaceAvailable, float relativeTotal) {
		int spaceUsed = 0;
		int components = parent.getComponentCount();
	 ...
	 */
	private GDimension getLayoutSize(GComponent parent, int type) {
		int width = 0;
		int height = 0;
		int components = parent.getComponentCount();
		int visibleComponents = getVisibleComponents(parent);
		...
		for (int i = 0; i < components; i++) {
			...
			GDimension d = getDimension(component, type);
	...
	private int getVisibleComponents(GComponent container) {
		int visibleComponents = 0;

		for (GComponent component : container.getComponents()) {
			if (component.isVisible())
				visibleComponents++;
		}
	...
	private GDimension getDimension(GComponent component, int type) {
		switch (type) {
		case PREFERRED:
	...
	public GDimension maximumLayoutSize(GComponent target) {
		return new GDimension(Integer.MAX_VALUE, Integer.MAX_VALUE);
	}

	/**
	 * Returns the alignment along the x axis. Use center alignment.
	 */
	public float getLayoutAlignmentX(GComponent parent) {
		return 0.5f;
	}
	 ...
	 */
	public float getLayoutAlignmentY(GComponent parent) {
		return 0.5f;
	}
	 ...
	 */
	public void invalidateLayout(GComponent target) {
